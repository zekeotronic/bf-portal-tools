...
import { BFPortalDiagnostics } from './diagnostics';
import { BFPortalCompletionProvider } from './completions';
import { BFPortalHoverProvider } from './hover';
import { TypeDefinitionParser } from './typeParser';

let typeParser: TypeDefinitionParser | null = null;
...
export function activate(context: vscode.ExtensionContext) {
  ...
  typeParser = new TypeDefinitionParser(typeDefsPath);

  const diagnostics = new BFPortalDiagnostics();
  const completionProvider = new BFPortalCompletionProvider(typeParser);
  const hoverProvider = new BFPortalHoverProvider(typeParser);

  // Register providers
  context.subscriptions.push(
    vscode.languages.registerCompletionItemProvider(
      ['typescript', 'bf-portal-ts'],
      completionProvider,
      '.',
      'm'
    )
        ...
        if (match && typeParser) {
          ...
          if (func) {
            const signature = new vscode.SignatureInformation(
              `mod.${func.name}(${func.params.map((p: any) => `${p.name}: ${p.type}`).join(', ')}): ${func.returnType}`
            );
            ...
            func.params.forEach((param: any) => {
              signature.parameters.push(
                new vscode.ParameterInformation(
                  ...
                  new vscode.MarkdownString(`**${param.name}** - Type: \`${param.type}\``)
                )
              );
            });
  ...
  context.subscriptions.push(
    vscode.workspace.onDidOpenTextDocument(doc => diagnostics.updateDiagnostics(doc)),
    vscode.workspace.onDidSaveTextDocument(doc => diagnostics.updateDiagnostics(doc)),
    vscode.workspace.onDidChangeTextDocument(e => diagnostics.updateDiagnostics(e.document))
  );
...
async function updateTypeDefinitions(context: vscode.ExtensionContext) {
...
async function autoSetupPortalProject(context: vscode.ExtensionContext) {
  const workspaceFolders = vscode.workspace.workspaceFolders;
  if (!workspaceFolders) return;

  for (const folder of workspaceFolders) {
    const files = await vscode.workspace.findFiles(
      new vscode.RelativePattern(folder, '**/*.portal.ts'),
      null,
      1
    );
    ...
    if (files.length > 0) {
      ...
      if (!fs.existsSync(tsconfigPath)) {
        const result = await vscode.window.showInformationMessage(
          'BF Portal scripts detected. Would you like to set up TypeScript configuration?',
          'Yes',
          'No'
        );
...
export function getTypeParser(): TypeDefinitionParser | null {
  return typeParser;
}